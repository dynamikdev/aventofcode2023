use std::{collections::HashMap, arch::global_asm, ops::Add};

const INPUT:&str="Game 1: 1 green, 1 blue, 1 red; 1 green, 8 red, 7 blue; 6 blue, 10 red; 4 red, 9 blue, 2 green; 1 green, 3 blue; 4 red, 1 green, 10 blue
...
const LIMIT_RED:u64 = 12;
const LIMIT_BLUE:u64 = 14;
const LIMIT_GREEN:u64 = 13;

fn filter_and_count_ids(input:&str) -> u64{
    let lines:Vec<&str> = input.split_whitespace().collect();
    let mut acc:u64= 0;
    'line : for line in lines{
        let (id,game_str) = id_and_game_extractor(line);
        let game = game_extractor(game_str);
        for res in game{
            if res.contains_key("blue") && res.get("blue").unwrap() > &LIMIT_BLUE{
                continue 'line;
            }
            if res.contains_key("green") && res.get("green").unwrap() > &LIMIT_GREEN{
                continue 'line;
            }
            if res.contains_key("red") && res.get("red").unwrap() > &LIMIT_RED{
                continue 'line;
            }
        }
        acc += id;
...
fn id_and_game_extractor(line:&str)-> (u64,&str){
    let game_info = line.split_once(':').unwrap().0;
    let id =  game_info.split_at(5).1.parse::<u64>().unwrap();
    let game = line.split_at(game_info.len()+1).1;
    (id,game)
}

fn game_extractor(line:&str)-> Vec<HashMap<&str, u64>>{
    let grabs: Vec<Vec<&str>> = line.split(';').map(|g| g.trim().split(',').map(|g| g.trim()).collect()).collect();
    println!("{:?}", grabs);
    let mut parsed_grabs:Vec<HashMap<&str, u64>> = vec![];
    for  grab in grabs{
        let mut parsed_grab:HashMap<&str, u64> = HashMap::new();
        for color in grab{
            let splited = color.split_once(' ').unwrap();
            parsed_grab.insert(splited.1, splited.0.parse::<u64>().unwrap());
        }
        parsed_grabs.insert(parsed_grabs.len(), parsed_grab);
    }
    println!("{:?}",parsed_grabs);
    todo!()
}
...
#[cfg(test)]
mod test {
    use crate::{filter_and_count_ids, id_and_game_extractor, game_extractor};
...
Game 5: 6 red, 1 blue, 3 green; 2 blue, 1 red, 2 green";
    ...
    #[test]
    fn final_res() {
        let res = filter_and_count_ids(INPUT);
        assert_eq!(res,8);
    }

    #[test]
    fn id_extractor_test() {
        let res = id_and_game_extractor("Game 1: 3 blue, 4 red; 1 red, 2 green, 6 blue; 2 green");
        assert_eq!(res.0,1);
        assert_eq!(res.1," 3 blue, 4 red; 1 red, 2 green, 6 blue; 2 green");
        let res = id_and_game_extractor("Game 15: 3 blue, 4 red; 1 red, 2 green, 6 blue; 2 green");
        assert_eq!(res.0,15);
        assert_eq!(res.1," 3 blue, 4 red; 1 red, 2 green, 6 blue; 2 green");
        let game_parsed = game_extractor(" 3 blue, 4 red; 1 red, 2 green, 6 blue; 2 green");
    }
